...
class Note(BaseClass):
    ...
    def set_theme(self) -> None:
        ...
        if len(sentences) == 0:
            ...
            filtered_tokens = [token for token in processed_text if token not in Note._stop_words and len(token) > 2]
            self.theme = " ".join(filtered_tokens[:3]) if filtered_tokens else ""
            return
        ...
        for sentence in sentences:
            ...
            filtered_tokens = [token for token in tokens if token not in Note._stop_words and len(token) > 2]
            if len(filtered_tokens) >= 2:  # Only include sentences with at least 2 meaningful words
        ...
        if len(processed_sentences) < 2:
            ...
            self.theme = " ".join(all_tokens[:3]) if all_tokens else ""
            return
        ...
        try:
            ...
            self.theme = " ".join(unique_topic_words[:5])  # Limit to 5 words max
            
        except Exception:
            # Fallback to most frequent words if topic modeling fails
            frequent_words = list(self.high_frequency_words.keys())[:3]
            self.theme = " ".join(frequent_words) if frequent_words else ""
